                ...
                {% if todoList.u_id == session.user_id %}
                    <tr>
                        ...
                        {% for usersList in usersArray %}
                            ...
                            {% endif %}
                        {% endfor %}
                    <td><a href=\"?id={{ todoList.id }}&action=edit\">Изменить</a>
                    {% if todoList.assigned_user_id == session.user_id %}
                        <a href=\"?id={{ todoList.id }}&action=done\">Выполнить</a>
                    {% endif %}
                    <a href=\"?id={{ todoList.id }}&action=delete\">Удалить</a></td>
                    <td>
                        <form method=\"POST\">
                            <select name=\"assigned_user_id\">
                    {% for usersList in usersArray %}
            ...
            {% for todoList in todoArray %}
                {% if todoList.assigned_user_id == session.user_id and todoList.user_id != session.user_id %}
                    <tr>
                        ...
                        <td>
                            <a href=\"?id={{ todoList.id }}&action=edit\">Изменить</a>
                            <a href=\"?id={{ todoList.id }}&action=done\">Выполнить</a>
                            <a href=\"?id={{ todoList.id }}&action=delete\">Удалить</a>
                        </td>
                    </tr>
                {% endif %}
            {% endfor %}
...
</div>", "task/list.twig", "/Applications/MAMP/htdocs/composer/application/views/task/list.twig");
    }
}
